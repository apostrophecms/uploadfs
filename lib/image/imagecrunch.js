      ...
      imagecrunch: function(callback) {
        ...
        proc.on('close', function(code) {
          if (code !== 0) {
            ...
            if (matches) {
              result.width = parseInt(matches[1], 10);
            }
            matches = output.match(/height: (\d+)/);
            if (matches) {
              result.height = parseInt(matches[1], 10);
            }
            matches = output.match(/originalWidth: (\d+)/);
            if (matches) {
              result.originalWidth = parseInt(matches[1], 10);
            }
            matches = output.match(/originalHeight: (\d+)/);
            if (matches) {
              result.originalHeight = parseInt(matches[1], 10);
            }
            matches = output.match(/orientation: (\w+)/);
            if (matches) {
              result.orientation = matches[1];
            }
            matches = output.match(/extension: (\w+)/);
            if (matches) {
              result.extension = matches[1];
            }
            if (!(result.width && result.height && result.extension)) {
              return callback('Unexpected output from imagecrunch: ' + output);
            }
            return callback(null);
          }
        });
