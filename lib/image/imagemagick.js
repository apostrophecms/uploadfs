  ...
  var self = {
    ...
    convert: function(context, callback) {
      ...
      function convertSvg(context, callback) {
        ...
        function both() {
          var scaleViewport = widthHeight.width / context.info.width;
          var widthViewport = context.crop.width * scaleViewport;
          var heightViewport = context.crop.height * scaleViewport;
          context.info.svg.$.width = widthViewport;
          context.info.svg.$.height = heightViewport;
          var scaleViewBox = viewBox.width / widthHeight.width;
          var top = viewBox.top + context.crop.top * scaleViewport * scaleViewBox;
          var left = viewBox.left + context.crop.left * scaleViewport * scaleViewBox;
          var width = context.crop.width * scaleViewport * scaleViewBox;
          var height = context.crop.height * scaleViewport * scaleViewBox;
          console.log('both');
          console.log(top, left, width, height, widthViewport, heightViewport);
          context.info.svg.$.viewBox = left + ' ' + top + ' ' + width + ' ' + height;
          var builder = new xml2js.Builder();
          var markup = builder.buildObject({ svg: context.info.svg });
          return markup;
