...
var optimizers = {};
var optimizerNames = [ 'imagemin-jpeg-recompress', 'imagemin-mozjpeg', 'imagemin-pngquant', 'imagemin-optipng' ];
...
_.each(optimizerNames, function(name) {
  try {
    optimizers[name] = require(name);
  } catch (e) {
...
optimizerNames = _.filter(optimizerNames, function(name) {
      ...
      function convertStandard(context, callback) {
        ...
        return spawnThen('convert', args, function (err) {
          ...
          imagemin(resizedPaths, context.tempFolder + '/', {
            plugins: _.map(optimizerNames, function(name) {
              return optimizers[name](optimizerOptions[name] || {});
            })
          })
            .then(function(files) {
