...
module.exports = function() {
  ...
  var self = {
    ...
    init: function(_options, callback) {
      options = _options;
      options.svgWidth = options.svgWidth || 1600;
      if (options.imagemin) {
        self.optimizers = {};
    ...
    identify: function(path, callback) {
      ...
      function sniffSvg(path) {
        try {
          var fd = require('fs').openSync(path, 'r');
          var buffer = new Buffer(16384);

          require('fs').readSync(fd, buffer, 0, 16384, null);
          ...
          data = data.trim();
          return data.match(/^\<svg/);
        } catch (e) {
          console.error(e);
          return false;
        }
      }
      function identifySvg(callback) {
        return require('fs').readFile(path, 'utf8', function(err, data) {
          if (err) {
            return callback(err);
          }
          return xml2js.parseString(data, function(err, svg) {
            if (err) {
              return callback(err);
            }
            if (!svg) {
              return callback(new Error('no xml data'));
            }
            if (!svg.svg) {
              return callback(new Error('wrong kind of xml file'));
            }
            svg = svg.svg;
            var coords = svgParseWidthHeight(svg);
            if (coords) {
              // The viewport is determined by width and height attributes.
              // We still have to scale it to the virtual svgWidth because
              // it would be incorrect to reject a "300x200" SVG as
              // "too small" for a use case, since they scale forever
              console.log('width/height based');
              return callback(null, {
                width: options.svgWidth,
                height: coords.height * options.svgWidth / coords.width,
                svg: svg,
                extension: 'svg'
              });
            }
            // No width and height, base it on the viewBox 
            coords = svgParseViewBox(svg);
            if (!coords) {
    ...
    convert: function(context, callback) {
      if (context.info.extension === 'svg') {
        return convertSvg(context, callback);
      } else if (context.info.animated) {
        if (options.gifsicle) {
      ...
      // SVG strategy
      //
      // 1. In identify, return a synthesized width and height, based on
      // a fixed width and a proportional height derived from viewBox and
      // the configured fixed width for uploadfs. If viewBox is not available,
      // do not return width and height.
      //
      // 2. If viewBox is unknown or we are not cropping, return as-is, copying
      // to all sizes. Else...
      //
      // 3. Scale the crop coordinates back down to the width/height of the
      // viewBox less the minx/miny. Then offset them by the minx/miny.

      function convertSvg(context, callback) {
        console.log('in convertSvg');
        console.log('CROP:', context.crop);
        if (!context.info.svg) {
          return justCopy(callback);
        }
        if (!context.crop) {
          return justCopy(callback);
        }
        var viewBox = svgParseViewBox(context.info.svg);
        var widthHeight = svgParseWidthHeight(context.info.svg);
        return scale(callback);
        function justCopy(callback) {
          console.log('justCopy');
          return async.series([
            copyOriginal,
            copySizes
          ], callback);
          function copyOriginal(callback) {
            context.adjustedOriginal = context.tempFolder + '/original.svg';
            return copyFile(context.workingPath, context.adjustedOriginal, callback);
          }
          function copySizes(callback) {
            return async.eachSeries(context.sizes, copySize, callback);
            function copySize(size, callback) {
              return copyFile(context.workingPath, context.tempFolder + '/' + size.name + '.svg', callback);
            }
          }
        }
        function scale(callback) {
          if (!widthHeight) {
            if (!viewBox) {
              ...
              return copyRendered(viewBoxOnly(), callback);
            }
          } else {
            if (!context.info.svg.$.viewBox) {
              return copyRendered(widthHeightOnly(), callback);
            } else {
              return copyRendered(both(), callback);
            }
          }
        }

        function copyRendered(markup, callback) {
          return async.series([
            copyOriginal,
            copySizes
          ], callback);
          function copyOriginal(callback) {
            ...
            context.adjustedOriginal = context.tempFolder + '/original.svg';
            return require('fs').writeFile(context.adjustedOriginal, markup, callback);
          }
          function copySizes(callback) {
            return async.eachSeries(context.sizes, copySize, callback);
            function copySize(size, callback) {
              return require('fs').writeFile(context.tempFolder + '/' + size.name + '.svg', markup, callback);
            }
          }
        }

        function viewBoxOnly() {
          console.log('viewBoxOnly');
          var scale = viewBox.width / context.info.width;
          var width = context.crop.width * scale;
          var height = context.crop.height * scale;
          var top = viewBox.top + context.crop.top * scale;
          var left = viewBox.left + context.crop.left * scale;
          console.log(top, left, width, height);
          context.info.svg.$.viewBox = left + ' ' + top + ' ' + width + ' ' + height;
          var builder = new xml2js.Builder();
          var markup = builder.buildObject({ svg: context.info.svg });
          return markup;
        }

        function widthHeightOnly() {
          var scale = widthHeight.width / context.info.width;
          var width = context.crop.width * scale;
          var height = context.crop.height * scale;
          var top = context.crop.top * scale;
          var left = context.crop.left * scale;
          console.log('widthHeightOnly');
          console.log(top, left, width, height);
          context.info.svg.$.viewBox = left + ' ' + top + ' ' + width + ' ' + height;
          delete context.info.svg.$.width;
          delete context.info.svg.$.height;
          var builder = new xml2js.Builder();
          var markup = builder.buildObject({ svg: context.info.svg });
          return markup;
        }
        ...
        function both() {
          var viewport = { width: context.info.width, height: context.info.height };
          
          // for translation between viewport and viewbox coordinate systems divide by these factors:
          var xScaleFactor = viewport.width / viewBox.width;
          var yScaleFactor = viewport.height / viewBox.height;
          console.log('both', viewport, context.info.width, context.info.height)
          
          // translate viewport selection to viewbox coordinates
          var _minX = context.crop.left / xScaleFactor;
          var _minY = context.crop.top / yScaleFactor;

          var _width = context.crop.width / xScaleFactor;
          var _height = context.crop.height / yScaleFactor;

          var translatedViewbox = [_minX, _minY, _width, _height];
          var translatedViewboxString = translatedViewbox.join(" ");
          console.log('xScale', xScaleFactor)
          console.log('yScale', yScaleFactor)
          console.log('viewport', viewport)
          console.log('original viewbox', viewBox)
          console.log('_minX', _minX)
          console.log('_minY', _minY)
          console.log('_width', _width)
          console.log('_height', _height)
          console.log('crop', context.crop)
          console.log('translated viewbox', translatedViewboxString)

          context.info.svg.$.viewBox = translatedViewbox.join(" ");
          var builder = new xml2js.Builder();
          var markup = builder.buildObject({ svg: context.info.svg });
          return markup;
        }
...
function svgParseViewBox(svg) {
  var coords = _.get(svg, '$.viewBox');
  if (!coords) {
    return false;
  }
  ...
  if (!parseFloat(coords[3])) {
    return false;
  }
  coords = {
    left: parseFloat(coords[1]),
    top: parseFloat(coords[0]),
...
function svgParseWidthHeight(svg) {
  var width = _.get(svg, '$.width');
  if (!width) {
    return false;
  }
