    ...
    identify: function(path, callback) {
      ...
      return async.series({
        identify: function(callback) {
          // Check magic number first to minimize attack surface of imagemagick, if it
          // does not smell like a file at least some browser accepts as an image we should
          // reject it out of hand
          let fd;
          try {
            fd = fs.openSync(path);
            const buffer = Buffer.alloc(16);
            const magic = fs.readSync(fd, buffer, 0, 16);
            const signatures = [
              // GIF
              [ 0, [ 0x47, 0x49, 0x46 ] ],
              // JPEG
              ...
              // WEBM
              [ 0, [ 0x52, 0x49, 0x46, 0x46 ], 8, [ 0x57, 0x45, 0x42, 0x50 ] ]
            ];
            let passed = false;
            for (const signature of signatures) {
              let good = true;
              for (let i = 0; (i < signature.length); i += 2) {
                const offset = signature[i];
                const bytes = signature[i + 1];
                for (let j = 0; (j < bytes.length); j++) {
                  if (buffer[offset + j] !== bytes[j]) {
                    good = false;
                    j = bytes.length;
                    i = signature.length;
                  }
            ...
            fs.closeSync(fd);
            if (!passed) {
              return callback(new Error('Signature does not match a valid web image format'));
            }
          } catch (e) {
            return callback(e);
          }
