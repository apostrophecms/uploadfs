...
var regexSvgType = /^(?:m|s)vg/; // Imagemagick sometimes detects SVGs as MVG...
...
module.exports = function() {
  ...
  return {
      ...
      return async.series({
        identify: function(callback) {
          return im(path).identify(function (err, _info) {
            ...
            if (regexSvgType.test(info.format.toLowerCase())) {
              info.format = 'svg';
              info.extension = 'svg';
            }
            return callback(null);
     ...
     * context object. (nb: Ignored for svg files)
     *
     ...
     *
     * svgoOptions: optional object with options for svgo optimizer.
     * See https://github.com/svg/svgo
     *
     * All images, including the "original" if copyOriginal is set, are
     * auto-rotated to the orientation expected by web browsers. (nb: Ignored for svg files)
     *
     * OUTPUT
     *
    ...
    convert: function(context, callback) {
      ...
      function convertSvg (context, callback) {
        var workingPath = context.workingPath

        // FIXME: Currently the all the files size are uploaded, but the file remain the same since the size does not make sense for svg
        // but, in order to keep only one file, we would have to refactor apostrophe-attachement too.
        var fileNames = context.sizes.map(function (size) {
        ...
        fs.readFile(workingPath)
          ...
          .then(function(buffer) {
            return Promise.all(fileNames.map(function (filename) {
              return fs.outputFile(context.tempFolder + '/' + filename, buffer)
            }))
          })
