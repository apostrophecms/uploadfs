    ...
    convert: function(context, callback) {
        ...
        if(context.crop) {
          return new Promise((resolve, reject) => {
            ...
            return image.crop(
              ...
              crop.width,
              crop.height,
              (err) => err ? reject(err) : resolve(image)
            )
          })
        }
      ...
      function copyOriginal(image) {
        ...
        return new Promise((resolve, reject) => {
          ...
          image.write(copyPath, (err) => err ? reject(err) : resolve(image))
        })
      }

      function sizes(image) {
        const sizeOperations = context.sizes.map(size => sizeOperation(image, size))
      ...
      function sizeOperation(image, size) {
        ...
        const width = Math.min(size.width, image.bitmap.width)
        const height = Math.min(size.height, image.bitmap.height)
        const quality = context.scaledJpegQuality ? context.scaledJpegQuality : 80
        ...
        return new Promise((resolve, reject) => {
          image
            ...
            .write(sizePath, (error) => error ? reject(error) : resolve(null))
        })
      }
    },
  }
