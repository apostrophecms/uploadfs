...
module.exports = function() {
  ...
  var self = {
    ...
    copyBlob: function(_src, dst, callback) {
      const blob = blobSvcs[0];
      
      const src = self.getUrl(_src);
      blob.svc.startCopyBlob(src, blob.container, dst, callback);
    },
    
    getUrl: function(path) {
      const blob = blobSvcs[0];
      const uri = blob.svc.getUrl(blob.container, path, null);
      return uri;
    }
  };
