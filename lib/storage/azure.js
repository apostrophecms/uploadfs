...
module.exports = function() {
  ...
  var self = {
    ...
    copyIn: function(localPath, _path, options, callback) {
      ...
      out.on('error', outErr => {
        if (outErr && !hasError) {
          console.log('Error in write stream', outErr);
          callback(outErr);

          // clean up
          fs.unlink(tempPath, outErrUnlinkErr => {
            if (outErrUnlinkErr) {
              // Failure here might mean no file exists ENOENT or other
              // We tried :)
              console.log("Trying to clean up after error creating gzip and got another error!", outErrUnlinkErr);
            }
          });
        }
      });
