  ...
  var self = {
    ...
    copyIn: function(localPath, _path, options, callback) {
      ...
      if (self.shouldGzip(fileExt)) {
        return self.gzipStream(path, tempPath, callback);
      } else {
        return self.createContainerBlobs(path, tempPath, callback);
      }
    },

    createContainerBlobs: function(path, tempPath, callback) {
    ...
    doGzip: function(path, tempPath, callback) {
      ...
      out.on('finish', function() {
        self.createContainerBlobs(path, tempPath, callback);
      });
    ...
    shouldGzip: function(ext) {
      return !self.gzipBlacklist.contains(ext);
    },
