...
module.exports = function() {
  ...
  var self = {
    ...
    copyIn: function(localPath, _path, options, callback) {
      ...
      const tmpDir = 'temp/out';
      const tmpFileName = Math.random().toString(36).substring(7);
      console.log(localPath, path, tmpDir, tmpFileName);
      
      // store gzipped file in temp dir 
      fs.mkdtemp(tmpDir, (e, dir) => {
        console.log('dirname', dir);
        if (e) {
          console.log(e);
          return callback(e);
        } else {
          const tempPath = dir + '/' + tmpFileName;
          ...
          out.on('finish', () => {
            console.log("Finish zip pipe");
            async.each(blobSvcs, (blobSvc, cb) => {
              createContainerBlob(blobSvc, path, tempPath, function(err) {
                if (err) {
                  __log('! Cannot create blob in container : ' + err);
                  cb(err);
                }
                
                removeLocalBlob(tempPath, e => {
                  cb(e); 
                });
              });
            }, err => {
