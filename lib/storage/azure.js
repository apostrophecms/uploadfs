...
module.exports = function() {
  ...
  var self = {
    ...
    catalog: function(prefix, callback) {
      ...
      return pass();
      function pass() {
        const blobSvc = self.blobSvcs[0];
        return blobSvc.svc.listBlobsSegmentedWithPrefix(blobSvc.container, prefix, continuationToken, function(err, data) {
          if (err) {
            return callback(err);
          }
          data.entries.forEach(function(item) {
            ...
            var matches = item.name.match(/^(.*)?-disabled-[0-9a-f]+$/);
            if (matches && (utils.getDisabledPath(matches[1], self.options.disabledFileKey) === item.name)) {
              list.push({
                path: matches[1].replace(new RegExp('^' + regexpQuote(prefix)), ''),
              ...
              list.push({
                path: item.name.replace(new RegExp('^' + regexpQuote(prefix)), ''),
                size: item.contentLength,
                updatedAt: item.lastModified,
                disabled: false
              });
    ...
    getUrl: function() {
      ...
      var url = blob.svc.getUrl(blob.container, '');
      return url;
    },
