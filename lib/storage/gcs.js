  ...
  const self = {
    ...
    copyIn: function(localPath, path, options, callback) {
      ...
      function upload(callback) {
        return client.bucket(bucketName).upload(localPath, uploadOptions, callback);
      }
      function makePublic(callback) {
        return client.bucket(bucketName).file(path).makePublic(callback);
      }
    },
    ...
    catalog: function(prefix, callback) {
      ...
      function getList(callback) {
        return client.bucket(bucketName).getFiles({
          prefix
        }, function(err, files) {
          if (err) {
            return callback(err);
      ...
      function getDisabled(callback) {
        return async.eachLimit(rawList, 5, function(item, callback) {
          return client.bucket(bucketName).file(item.name).isPublic(function(err, p) {
            if (err) {
              return callback(err);
            }
            list.push({
