  ...
  var self = {
    ...
    enable: function(path, callback) {
      if (self.options.disabledFileKey) {
        return fs.rename(uploadsPath + utils.getDisabledPath(path), uploadsPath + path, callback);
      } else {
    ...
    disable: function(path, callback) {
      if (self.options.disabledFileKey) {
        return fs.rename(uploadsPath + path, uploadsPath + utils.getDisabledPath(path), callback);
      } else {
