...
module.exports = function() {
  ...
  var self = {
    ...
    catalog: function(prefix, callback) {
      var list = [];
      return spelunk(prefix, function(err) {
        if (err) {
          return callback(err);
        }
        list.sort(function(a, b) {
      ...
      function spelunk(folder, callback) {
        return fs.readdir(uploadsPath + folder, function(err, files) {
          if (err) {
            return callback(err);
          }
          return async.eachLimit(files, 5, function(file, callback) {
            var path = folder + '/' + file;
            return fs.stat(uploadsPath + path, function(err, stats) {
              var matches;
              if (err) {
                return callback(err);
              }
              if (stats.isDirectory()) {
                return spelunk(path, callback);
              } else {
                if (self.options.disabledFileKey) {
                  matches = file.match(/^(.*)?-disabled-[0-9a-f]+$/);
                  if (matches && (utils.getDisabledPath(folder + '/' + matches[1]) === path)) {
                    list.push({
                      path: folder + '/' + matches[1],
                      trash: true
                    });
                  } else {
                    list.push({
                  ...
                  list.push({
                    path: folder + '/' + file,
                    trash: (stats.mode & parseInt('777', 8)) === self.options.disablePermissions
                  });
                }
                return callback(null);
              }
