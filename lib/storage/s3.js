...
module.exports = function() {
  ...
  var self = {
    init: function (options, callback) {
      ...
      // CORS
      client.getBucketCors(getCorsConfig, function (err, data) {
        if (err) {// an error occurred
          console.log(err, err.stack);
          return callback(null);
        } else { // successful response
          if (data.CORSRules !== putCorsConfig.CORSConfiguration.CORSRules) {
            client.putBucketCors(putCorsConfig, function (err, data) {
              if (err) {// an error occurred
                console.log(err, err.stack);
                return callback(null);
