...
const testCopyImageInCrop = cb => {
  ...
  uploadfs.copyImageIn(
    ...
    (e, info) => {
      if (e) fail(6, e)
      if (info.basePath !== '/images/profiles/me-cropped') fail(6, 'info.basePath is incorrect')
      pass(6)
      ...
      uploadfs.remove(`${basePath}-cropped.${ext}`, e => {
        async.each(
          ...
          (size, cb) => {
            ...
            if (!fs.statSync(`test${name}`).size) fail(8, 'cannot stat scaled/copied image')
...
const testCopyImageIn = cb => {
  ...
  uploadfs.copyImageIn(img, basePath, (e, info) => {
    if (e) fail(2, e)
    if (info.basePath !== '/images/profiles/me') fail(2, 'info.basePath is incorrect')
    pass(2)
    ...
    if (!fs.statSync(`test/images/profiles/me.${ext}`).size) fail(4, 'cannot stat copied image')
    pass(4)
    ...
    uploadfs.remove(`/images/profiles/me.${ext}`, e =>
      async.each(
        ...
        (size, cb) => {
          const name = `${info.basePath}.${size.name}.${ext}`
          if (!fs.statSync(`test${name}`).size) fail(5, 'cannot stat scaled/copied image')
...
run(() => {
  ...
  config.postprocessors = [
    {
      postprocessor: (files, folder, options = { test: false }) => {
        ...
        if (!options.test) fail('Postprocessor', 'postprocessor did not receive options')
        if (!files) fail('Postprocessor', 'did not receive files array')
        if (!files.length) return Promise.resolve(true)
        if (!files[0].match(/\.(gif|jpg|png|webp)$/))
          fail(c, `invalid file extension: ${files[0]}`)
        if (!fs.existsSync(files[0])) fail('Postprocessor', `cannot locate file: ${files[0]}`)
  ...
  run(() => {
      if (!filesSeen) fail(0, 'postprocessor saw no files')
